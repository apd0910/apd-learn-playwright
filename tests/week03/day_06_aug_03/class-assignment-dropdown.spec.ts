...
test(`Learn to Handle DropDown`,async ({page}) => {
    ...
    //Get the values from the preferred currency dropdown
    const preferred_currency_dropdown = page.locator(`#createLeadForm_currencyUomId > option`)
    const preferred_currency_dropdown_count = await preferred_currency_dropdown.count()

    console.log(`No. of preferred currency - ${preferred_currency_dropdown_count}`)

    for (let index = 0; index < preferred_currency_dropdown_count; index++) {
        console.log(await preferred_currency_dropdown.nth(index).innerText())// Print all the currency types
    }
    await page.waitForTimeout(5000)
